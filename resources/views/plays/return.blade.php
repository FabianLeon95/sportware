...
@section('content')
    <div class="card">
        ...
        <div class="card-content">
            <form method="POST" action="{{ route('plays.return.create', $match) }}">
                @csrf
                ...
                <div class="input-field col s12">
                    <select id="runner" name="runner">
                        @foreach($players as $player)
                            <option value="{{ $player->id }}" selected>#{{ $player->shirt_number }} {{ ($player->user) ? $player->user->name: '' }}</option>
                        @endforeach
                    </select>
                    <label for="team">Runner</label>
                </div>
                ...
                <div class="row mt-5 center-align">
                    <div class="col s4">
                        <label>
                            <input type="checkbox" name="specials[]" id="fair_catch" value="fair_catch"/>
                            <span>Fair Catch</span>
                        </label>
                    </div>
                    <div class="col s4">
                        <label>
