        ...
        <div class="card-content">
            <div class="row mb-0">
                <div class="col s6 p-0">
                    <span class="card-title">Pass</span>
                </div>
            </div>
        </div>
        <div class="card-content">
            <form method="POST" action="{{ route('plays.pass.create', $match) }}">
                @csrf
                        ...
                        <div class="col s3">
                            <label>
                                <input name="status" type="radio"
                                       value="{{ $status->id }}" {{ $status->id == 1 ? 'checked' : '' }} />
    ...
    <script>
        $(document).ready(function () {
            ...
            slider.noUiSlider.on('change', function () {
                ...
                if ($('input[type=radio][name=status]:checked').val() === 'incomp') {
                    $('#touch_down').prop("checked", false)
                    this.set($('#ball_on').val());
                }
            ...
            $('input[type=radio][name=status]').change(function () {
                if (this.value === 'incomp') {
                    $('#touch_down').prop("checked", false);
                    slider.noUiSlider.set($('#ball_on').val());
                }
            });
